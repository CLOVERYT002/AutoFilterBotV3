...
async def filter(client, message):
    ...
    if AUTH_CHANNEL:
        ...
        try:
            ...
            await client.send_message(
                chat_id=message.from_user.id,
                text="**Please Join Main Channel to use this Bot!**",
    ...
    if 2 < len(message.text) < 100:    
        ...
        buttons.append(
            [InlineKeyboardButton(text="⭕️NEXT ⭕️",callback_data=f"next_0_{keyword}")]
        )    
        buttons.append(
    ...
    if (clicked == typed):
        ...
        if query.data.startswith("next"):
            ...
            if int(index) == int(data["total"]) - 2:
                ...
                buttons.append(
                    [InlineKeyboardButton("😒 BACK😒", callback_data=f"back_{int(index)+1}_{keyword}"),InlineKeyboardButton("⭕️NEXT ⭕️", callback_data=f"next_{int(index)+1}_{keyword}")]
                )
                buttons.append(
                    [InlineKeyboardButton(f"📝 Pages {int(index)+2}/{data['total']}", callback_data="pages")]
                )
            ...
            if int(index) == 1:
                ...
                buttons.append(
                    [InlineKeyboardButton("⭕️NEXT ⭕️", callback_data=f"next_{int(index)-1}_{keyword}")]
                )
                buttons.append(
